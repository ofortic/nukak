...
import 'package:nukak_maku/widgets/report_list.dart';
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data.role == 'admin') {
              return UReports(firebaseUser: firebaseUser);
            } else {
              return Text('no');
            }
...
class UReports extends StatelessWidget {
  const UReports({
    Key key,
      ...
      body: StreamBuilder(
          ...
          builder: (context, AsyncSnapshot<List<Report>> snapshot) {
            ...
            return Column(
              children: <Widget>[
                Expanded(
                  child: ReportList(reports: snapshot.data),
                ),
                IconButton(
